...
class YonghuController extends CommonController {
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
            $yonghuzhanghao = $postData['yonghuzhanghao'];
            $resultInfo = table_sql("select yonghuzhanghao from `yonghu` where `yonghuzhanghao` = '".$yonghuzhanghao."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"用户账号已存在"]));
            }
            foreach ($postData as $key => $value){
    ...
    public function update(){
        ...
        $postData = json_decode($tmpData,true);
            $yonghuzhanghao = $postData['yonghuzhanghao'];
            $id = $postData['id'];
            $resultInfo = table_sql("select yonghuzhanghao from `yonghu` where id <> ".$id." and `yonghuzhanghao` = '".$yonghuzhanghao."'");
            if ($resultInfo->num_rows > 0) {
                exit(json_encode(['code'=>500,'msg'=>"用户账号已存在"]));
            }
        $v = array();
